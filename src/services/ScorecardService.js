...
async function validateScorecardDetails (details) {
  ...
  if (sum > constants.MAX_SUM_OF_WEIGHTS) {
    throw new errors.BadRequestError(`Sum of weights is ${
      sum}, it should not be larger than ${constants.MAX_SUM_OF_WEIGHTS}.`)
  }
}
...
create.schema = {
  ...
  data: Joi.object().keys({
    ...
    scorecardDetails: Joi.array().items(Joi.object().keys({
      scoreSystemId: Joi.id(),
      weight: Joi.number().min(0).max(constants.MAX_WEIGHT).required()
    })).default([])
  }).required()
}
