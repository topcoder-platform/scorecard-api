...
const { Scorecard } = require('../../src/models')
const service = require('../../src/services/ScorecardService')
const helper = require('../../src/common/helper')
const commonData = require('./common/CommonData')
const testData = require('./common/ScorecardServiceData')
const expect = chai.expect
chai.use(sinonChai)

describe('scorecard service test', () => {
  ...
  it('T02:Search scorecard with criteria', async () => {
  ...
  it('T03:Get scorecard', async () => {
    ...
    const result = await service.getScorecard(data.id)
    expect(stubGetExtra).to.have.been.calledOnce
    expect(stubGetExtra).to.have.been.calledWithExactly(testData.index, data.id)
    expect(result).to.deep.eql(data.esResult)
  })
  it('T04:Create scorecard', async () => {
    ...
    const result = await service.createScorecard(commonData.userAdmin, data.data)
    expect(stubScanAll).to.have.been.calledOnce
    expect(stubScanAll).to.have.been.calledWithExactly(Scorecard, data.options)
  ...
  it('T05:Create scorecard with wrong weights', async () => {
    const data = testData.T05
    let error
    try {
      await service.createScorecard(commonData.userAdmin, data.data)
    } catch (err) {
      error = err
    }
  ...
  it('T06:Create scorecard with duplicated title', async () => {
  ...
  it('T07:Fail to create scorecard', async () => {
    ...
    try {
      ...
      user.sub = 'admin'
      await service.createScorecard(user, data.data)
    } catch (err) {
      error = err
    }
    expect(stubScanAll).to.have.been.calledOnce
  ...
  it('T08:Partially update scorecard', async () => {
    ...
    const result = await service.partiallyUpdateScorecard(commonData.userAdmin, data.id, data.data)
    expect(stubScanAll).to.have.been.calledOnce
    expect(stubScanAll).to.have.been.calledWithExactly(Scorecard, data.options)
  ...
  it('T09:Partially update scorecard 2', async () => {
  ...
  it('T10:Fail to update scorecard', async () => {
    ...
    try {
      ...
      user.handle = undefined
      user.sub = 'admin'
      await service.partiallyUpdateScorecard(user, data.id, data.data)
    } catch (err) {
      error = err
    }
    expect(stubGetById).to.have.been.calledOnce
  ...
  it('T11:Fully update scorecard', async () => {
    const data = testData.T11
    const stubGetById = sinon.stub(helper, 'getById').callsFake(async () => data.oldValue)
    const stubUpdate = sinon.stub(helper, 'update').callsFake(async () => data.result)
    const stubUpdateExtra = sinon.stub(ESClient, 'updateExtra').callsFake(async () => undefined)
    const result = await service.fullyUpdateScorecard(commonData.userAdmin, data.id, data.data)
    expect(stubGetById).to.have.been.calledOnce
    expect(stubUpdate).to.have.been.calledOnce
    data.data.updatedBy = 'admin'
    expect(stubUpdate).to.have.been.calledWithExactly(data.oldValue, data.data)
  ...
  it('T12:Delete scorecard', async () => {
    ...
    const result = await service.deleteScorecard(data.id)
    expect(stubGetById).to.have.been.calledOnce
    expect(stubRemove).to.have.been.calledOnce
  ...
  it('T13:Fail to delete review process', async () => {
    ...
    let error
    try {
      await service.deleteScorecard(data.id)
    } catch (err) {
      error = err
    }
    expect(stubGetById).to.have.been.calledOnce
    ...
    expect(stubDeleteExtra).to.have.been.calledWithExactly(testData.index, data.id)
    expect(stubCreate).to.have.been.calledOnce
    expect(stubCreate).to.have.been.calledWithExactly(Scorecard, data.value.originalItem())
    expect(error.message).to.eql('test')
  })
})
